...
class Create1920sPlayerStepSevenActivity : ComponentActivity() {
    ...
    fun updateCharacterListAndJson(){
        var tempStirng : String = "{\"Characters\" : \n[\n"
        val filename = "Characters.json"
        val output : OutputStream
        for(i in characterList.indices){
            if(i < characterList.size) {
                tempStirng += characterList[i].createJson() + ",\n"
            } else{
                tempStirng += characterList[i].createJson()
            }
        }
        openFileOutput(filename, Context.MODE_PRIVATE).use {
            it.write(tempStirng.toByteArray())
        }
    }
    ...
    fun createCharacterList(){
        try {
            val json : String
            val  inputStream: InputStream = assets.open("Characters.json")
